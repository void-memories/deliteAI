 ...
 *
 * This class contains all the necessary parameters and settings required to initialize
 * and configure the NimbleNet SDK. It includes authentication credentials, server endpoints,
 * resource limits, and deployment options.
 *
 * ## Parameters
 *
 ...
 *
 * @param clientId Unique client identifier provided by NimbleNet platform.
 *                Must be a non-empty string obtained from your NimbleNet account.
 * @param clientSecret Authentication secret key for API access.
 *                    Keep this value secure and never expose it in logs or client-side code.
 * @param host The base URL of the NimbleNet platform API endpoint.
 *            Must be a valid HTTPS URL (e.g., "https://api.nimblenet.ai").
 * @param deviceId Unique identifier for this device or application installation.
 *                Can be a UUID, device fingerprint, or custom identifier.
 *                Should remain consistent across app sessions for the same device.
 * @param debug Enable debug mode for detailed logging and diagnostics.
 *             Set to false in production builds for optimal performance.
 *             Default: false
 * @param initTimeOutInMs Maximum time in milliseconds to wait for SDK initialization.
 *                       Default: [SDK_CONSTANTS.INIT_DEFAULT_TIMEOUT]
 * @param compatibilityTag Version identifier for API compatibility checking.
 *                        Should match your application version or SDK compatibility version.
 *                        Used to ensure client-server API compatibility.
 * @param sessionId Optional custom session identifier for session management.
 *                 If empty, the SDK will generate a default session ID.
 *                 Useful for multi-user applications or session persistence.
 *                 Default: "" (empty string)
 * @param maxDBSizeKBs Optional maximum database size limit in kilobytes.
 *                    When reached, older data may be purged to stay within limits.
 *                    Set based on your device storage constraints.
 *                    Default: null (no specific limit)
 * @param maxEventsSizeKBs Optional maximum event data storage limit in kilobytes.
 *                        Controls how much event data can be cached before upload.
 *                        Default: null (no specific limit)
 * @param cohortIds Array of cohort identifiers for A/B testing and experimentation.
 *                 Used to assign users to specific test groups or feature variants.
 *                 Default: empty array
 * @param libraryVariant Deployment variant determining how the SDK is packaged and loaded.
 *                      Choose based on your app distribution and size requirements.
...
class NimbleNetConfig(
     ...
     *
     * This method serializes the configuration to JSON format, which can be useful for
     * logging, debugging, persistence, or network transmission.
     *
